...
Stratégie K-Fold recommandée pour un PFE
-----------------------------------------
  27 patients
  ├── 5  patients → test final (hold-out, séparés AVANT tout)
  └── 22 patients → K-Fold cross-validation (k=5)
       Fold 1 : train=18, val=4
       Fold 2 : train=18, val=4
       Fold 3 : train=18, val=4
       Fold 4 : train=18, val=4
       Fold 5 : train=18, val=4

Data augmentation : uniquement sur le train set de chaque fold.
Métriques         : Dice + IoU slice-averaged, combined score.
Modèle final      : moyenne des poids des 5 folds (model ensembling).

Usage
-----
# Entraînement K-Fold complet :
python inference.py train --epochs 50 --augment

# Inférence sur un patient :
python inference.py infer --input path/to/patient.nii.gz --output results/

# Inférence avec un fold spécifique :
python inference.py infer --input path/to/patient.nii.gz --fold 2
"""
...
K_FOLDS      = 5
N_TEST_HOLD  = 5          # patients réservés pour le test final
SEED         = 42
...
def train_one_fold(
    ...
    model     = unet_files.build_model().to(device)
